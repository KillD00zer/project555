...
|---|---|---|---|
| X8 | Firm Size — حجم الشركة | 2=كبيرة، 1=متوسطة، 0=صغيرة | Nominal |
...
2. **هل يختلف النمط الكلي للإدراك (X1+X5+X6 معاً) باختلاف حجم الشركة (X8)؟**  
...
| البند | التفاصيل |
|---|---|
| **السؤال** | هل يختلف متوسط مستوى الرضا (X10) باختلاف حجم الشركة (كبيرة / متوسطة / صغيرة) (X8)؟ |
| **IV (المستقل)** | X8 — Firm Size (3 فئات: كبيرة / متوسطة / صغيرة) |
| **DV (التابع)** | X10 — Satisfaction Level (0–10) |
| **H₀** | µ_كبيرة = µ_متوسطة = µ_صغيرة |
| **H₁** | يوجد اختلاف واحد على الأقل بين المتوسطات |
...
| البند | التفاصيل |
|---|---|
| **السؤال** | هل يختلف النمط الكلي للإدراك (X1+X5+X6 معاً) باختلاف حجم الشركة (X8)؟ |
| **IV (المستقل)** | X8 — Firm Size (كبيرة / متوسطة / صغيرة) |
| **DVs (التابعة)** | X1 (سرعة التسليم) + X5 (الخدمة) + X6 (صورة فريق المبيعات) |
| **H₀** | لا يوجد فرق في الـ Centroid بين المجموعات الثلاث |
| **H₁** | يوجد فرق في الـ Centroid |
| **اختبارات** | Box's M Test + Wilks' Lambda / Pillai's Trace + Univariate Tests لكل DV |

**مبرر الاختيار:** عندنا أكثر من DV واحدة مترابطة (X1, X5, X6 كلها تقيس إدراك العميل لـ HATCO)، MANOVA أدق من 3 ANOVAs منفصلة لأنه يحافظ على α ويتجنب Type I Error inflation (كما شرح الدكتور في المحاضرة 4).
...
- [ ] **MANOVA:**
  - Box's M Test
  - Wilks' Lambda / Pillai's Trace (Multivariate Test)
...
SEC 3A  │ ONE-WAY ANOVA
        ...
        │
SEC 3B  │ MANOVA
        │   1. Wilks' Lambda / Pillai's Trace (manual)
